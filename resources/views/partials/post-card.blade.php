                ...
                <a href="{{ route('profile.show', $post->user) }}" onclick="event.stopPropagation()" style="display: block; position: relative;">
                    ...
                    @auth
                        @if($post->user_id != auth()->id() && !auth()->user()->isFollowing($post->user_id))
                            <button 
                                onclick="event.stopPropagation(); event.preventDefault(); followUser({{ $post->user->id }})"
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-6 text-gray-500">
                    ...
                    @auth
                        ...
                        >
                            <svg id="like-icon-{{ $post->id }}" class="w-4 h-4 {{ $post->isLikedBy(auth()->user()) ? 'fill-current' : '' }}" fill="{{ $post->isLikedBy(auth()->user()) ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
